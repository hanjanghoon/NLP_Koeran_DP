...
import numpy as np
from torch.nn import ParameterList, Parameter ,Module   ##JH
...
class Embedder(Module):
    def __init__(self, model_dir, batch_size=64):
        ...
        self.scalar_parameters = ParameterList(
            [Parameter(torch.FloatTensor([0.3]), requires_grad=True) for i in range(3)])
    ...
    def sents2elmo(self, sents, output_layer=7):
        ...
        for w, c, lens, masks, texts in zip(test_w, test_c, test_lens, test_masks, test_text):
            ...
            for i, text in enumerate(texts):
                ...
                if output_layer == -1:
                    ...
                    pieces = []
                    for weight, tensor in zip(normed_weights, data):
                        # tensor=torch.from_numpy(tensor)
                        pieces.append(weight * tensor)
                    payload = self.gamma * sum(pieces)
                else:
