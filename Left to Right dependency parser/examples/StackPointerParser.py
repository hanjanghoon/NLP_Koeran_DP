  ...
  #  optim = generate_optimizer(opt, lr, network.parameters())
    ...
    for epoch in range(1, num_epochs + 1):
        ...
        for batch in range(1, num_batches + 1):
            ...
            word, char, pos, heads, types, masks_e, lengths_e, word_bert = input_encoder
            inputidx, answer_head, answer_types, previous, nexts, masks_d, lengths_d = input_decoder

            optim.zero_grad()

            # L2R
            loss_arc, loss_type, loss_cov, num = network.loss(inputidx, answer_head,
                                                              answer_types, previous, nexts,
                                                              mask_e=masks_e,
                                                              length_e=lengths_e, mask_d=masks_d,
                                                              length_d=lengths_d,
                                                              input_word_bert = word_bert)

            loss = loss_arc + loss_type + cov * loss_cov
            loss.backward()
            clip_grad_norm_(network.parameters(), clip) #######JH
            optim.step()
            ...
            if batch % 10 == 0:
                ...
                log_info = 'train: %d/%d, loss %.4f, arc: %.4f, type: %.4f, coverage: %.4f, time left (estimated): %.2fs'\
                           %(batch, num_batches, err, err_arc, err_type, err_cov, time_left)
        ...
        #cntbatch=0
        for i,batch in tqdm(enumerate(conllx_stacked_data.iterate_batch_stacked_variable(data_dev, batch_size, pos_embedding, type='dev', elmo=use_elmo, bert=use_bert))):
            #cntbatch+=1
            #print(cntbatch)
            input_encoder, input_decoder = batch

            word, char, pos, heads, types, masks, lengths, word_bert = input_encoder
            #inputidx, reverse_inputidx, answer_head, answer_types, reverse_previous, reverse_nexts, masks_d, lengths_d = input_decoder

            heads_pred, types_pred, _, _ = network.decode( mask=masks, length=lengths, beam=beam,
