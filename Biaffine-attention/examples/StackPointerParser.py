...
import neuronlp2.io.reader #hoon : bert
...
def main():
    ...
    args_parser.add_argument('--clip', type=float, default=1.0, help='gradient clipping')
    ...
    args_parser.add_argument('--bert_path', help='path for bert embedding model.')  # yjyj
    args_parser.add_argument('--bert_dim', type=int, help='dimension for bert embedding model') # yjyj
    args_parser.add_argument('--bert_learning_rate', type=float, default=5e-5, help='Bert Learning rate')
    args_parser.add_argument('--self_attention', action='store_true', help='use self_attention model instead of rnn.')  # true if use bert(hoon)
    args_parser.add_argument('--sa_path', help='path for config file of self-attention model.')  # yjyj

    args_parser.add_argument('--decode', choices=['mst', 'greedy'], help='decoding algorithm', required=True) #yj
    ...
    network = BiRecurrentConvBiAffine(word_dim, num_words, char_dim, num_chars, pos_dim, num_pos, num_filters, window,
                                      mode, hidden_size, encoder_layers, num_types, arc_space, type_space,
                                      ...
                                      elmo=use_elmo, elmo_path=elmo_path, elmo_dim=elmo_dim, bert = use_bert, bert_path=bert_path,
                                      bert_dim=bert_dim, self_attention=self_attention, sa_path=sa_path)
    ...
    def save_args():
        ...
        arguments = [word_dim, num_words, char_dim, num_chars, pos_dim, num_pos, num_filters, window,
                     mode, hidden_size, encoder_layers,
                     num_types, arc_space, type_space, pos_embedding]
        kwargs = {'p_in': p_in, 'p_out': p_out, 'p_rnn': p_rnn, 'biaffine': True, 'pos': use_pos, 'char': use_char, 'elmo': use_elmo, 'bert': use_bert}
    ...
    def generate_optimizer(opt, lr, params):
        ...
        if True:
            return BertAdam(params, lr=lr, betas=betas, weight_decay=gamma)
        if opt == 'adam':
    ...
    def generate_differentlr_bert_optimizer(lr, model):
        no_decay = ['bias', 'LayerNorm.weight']
        optimizer_grouped_parameters = [
            {'params': [p for n, p in model.named_parameters() if 'bert_model1' not in n and 'bert_model2' not in n ]
            }
        ]
    ...
    lr = learning_rate
    if use_bert:
        # scheduler, optim = generate_bert_optimizer(len(data_train) * num_epochs, lr, network)
        #optim = generate_differentlr_bert_optimizer( lr,network) # 1번
        optim = generate_old_bert_optimizer(len(data_train) * num_epochs, lr, network) # 2번
    ...
    for epoch in range(1, num_epochs + 1):
        ...
        for batch in range(1, num_batches + 1):
            # load data
            input_encoder, _ = conllx_stacked_data.get_batch_stacked_variable(data_train, batch_size, pos_embedding, unk_replace=unk_replace, elmo = use_elmo, bert = use_bert)
            ...
            # yjyj
            loss_arc, loss_type = network.loss(word, char, pos, heads, types, mask=masks, length=lengths,input_word_elmo=word_elmo,  input_word_bert=word_bert)
        ...
        dev_total_root = 0.0
        dev_total_inst = 0.0
        for batch in conllx_stacked_data.iterate_batch_stacked_variable(data_dev, batch_size, pos_embedding, type='dev', elmo=use_elmo):
            ...
            if use_elmo:
                ...
                heads_pred, types_pred = decode(word, char, pos, mask=masks, length=lengths,
                                                leading_symbolic=conllx_stacked_data.NUM_SYMBOLIC_TAGS, input_word_bert=word_bert, input_word_elmo=word_elmo)
        ...
        if dev_ucorrect_nopunc * 1.5 + dev_lcorrect_nopunc < dev_ucorr_nopunc * 1.5 + dev_lcorr_nopunc:
            ...
            if test_path:
                ...
                for batch in conllx_stacked_data.iterate_batch_stacked_variable(data_test, batch_size, pos_embedding, type='dev'):
                    ...
                    heads_pred, types_pred = decode(word, char, pos, mask=masks, length=lengths,
                                                    leading_symbolic=conllx_stacked_data.NUM_SYMBOLIC_TAGS, input_word_elmo=word_elmo)
                    word = word.data.cpu().numpy()
                    pos = pos.data.cpu().numpy()
                    lengths = lengths.cpu().numpy()
                    heads = heads.data.cpu().numpy()
