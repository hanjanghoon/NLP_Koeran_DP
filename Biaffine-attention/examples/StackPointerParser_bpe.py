...
def main():
    ...
    network = BiRecurrentConvBiAffine(word_dim, num_words, char_dim, num_chars, pos_dim, num_pos, num_filters, window,
                                      ...
                                      elmo=use_elmo, elmo_path=elmo_path, elmo_dim=elmo_dim, bert = use_bert, bert_path=bert_path, bert_dim=bert_dim)
    ...
    for epoch in range(1, num_epochs + 1):
        ...
        for batch in range(1, num_batches + 1):
            ...
            # yjyj
            loss_arc, loss_type, bert_word_feature_ids, bert_morp_feature_ids = network.loss(word, char, pos, heads, types, mask=masks, length=lengths, input_word_bert=word_bert)
        ...
        for batch in conllx_stacked_data.iterate_batch_stacked_variable(data_dev, batch_size, pos_embedding, type='dev', elmo=use_elmo):
            ...
            if use_elmo:
                ...
                word, char, pos, heads, types, masks, lengths, word_bert = input_encoder
                heads_pred, types_pred, bert_word_feature_ids, bert_morp_feature_ids = decode(word, char, pos, mask=masks, length=lengths,
                                                leading_symbolic=conllx_stacked_data.NUM_SYMBOLIC_TAGS, input_word_bert=word_bert)
            ...
            stats, stats_nopunc, stats_root, num_inst = parser_bpe.eval(word, pos, heads_pred, types_pred, heads, types, word_alphabet, pos_alphabet, lengths, punct_set=punct_set,
                                                                        symbolic_root=True, bert_word_feature_ids=bert_word_feature_ids, bert_morp_feature_ids=bert_morp_feature_ids)
        ...
        if dev_ucorrect_nopunc * 1.5 + dev_lcorrect_nopunc < dev_ucorr_nopunc * 1.5 + dev_lcorr_nopunc:
            ...
            if test_path:
                ...
                for batch in conllx_stacked_data.iterate_batch_stacked_variable(data_test, batch_size, pos_embedding, type='dev'):
                    ...
                    stats, stats_nopunc, stats_root, num_inst = parser_bpe.eval(word, pos, heads_pred, types_pred, heads, types, word_alphabet, pos_alphabet, lengths, punct_set=punct_set, symbolic_root=True)
                    ucorr, lcorr, total, ucm, lcm = stats
                    ucorr_nopunc, lcorr_nopunc, total_nopunc, ucm_nopunc, lcm_nopunc = stats_nopunc
                    corr_root, total_root = stats_root
