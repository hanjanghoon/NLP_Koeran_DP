...
def is_punctuation(word, pos, punct_set=None):
    if punct_set is None:
        ...
        return True if sum([p in punct_set for p in pos]) == len(pos) else False
    else:
        return pos in punct_set


def eval(words, postags, heads_pred, types_pred, heads, types, word_alphabet, pos_alphabet, lengths,
         punct_set=None, symbolic_root=False, symbolic_end=False, bert_word_feature_ids=None, bert_morp_feature_ids=None):
    ...
    for i in range(batch_size):
        ...
        for j in range(start, lengths[i] - end):
            word = [word_alphabet.get_instance(w).encode('utf-8') for w in words[i, j]]
            pos = [pos_alphabet.get_instance(p).encode('utf-8') for p in postags[i, j]]
            if bert_word_feature_ids[i][j] != 1 or bert_morp_feature_ids[i][j] != 1:
                continue
            total += 1
            ...
            # QUESTION: what is this checking?
...
def decode_MST(energies, lengths, leading_symbolic=0, labeled=True):
    ...
    for i in range(batch_size):
        ...
        for ch, pr in list(final_edges.items()):
            par[ch] = pr
            if labeled and ch != 0:
