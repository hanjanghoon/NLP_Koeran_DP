    ...
    while inst is not None and (not max_size or counter < max_size):
        ...
        for bucket_id, bucket_size in enumerate(buckets):
            if inst_size < bucket_size:
                ...
                if elmo:
                    data[bucket_id].append([sent.word_ids, sent.char_id_seqs, inst.pos_ids, inst.heads, inst.type_ids, sent.words, stacked_heads, children, siblings, stacked_types, skip_connect])
                else:
...
def read_stacked_data_to_variable(source_path, word_alphabet, char_alphabet, pos_alphabet, type_alphabet, pos_embedding,
    ...
    for bucket_id in range(len(buckets)):
        ...
        for i, inst in enumerate(data[bucket_id]):    # data is a list
            if elmo:
                wids, cid_seqs, pids, hids, tids, w, stack_hids, chids, ssids, stack_tids, skip_ids = inst
...
# dev, test load시 사용
def iterate_batch_stacked_variable(data, batch_size, pos_embedding, unk_replace=0., shuffle=False, type=None, elmo=False):
