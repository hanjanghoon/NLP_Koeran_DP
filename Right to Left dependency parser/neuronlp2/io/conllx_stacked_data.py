    ...
    while inst is not None and (not max_size or counter < max_size):
        ...
        for bucket_id, bucket_size in enumerate(buckets):
            if inst_size < bucket_size:
                stacked_heads, children, siblings, stacked_types, skip_connect, previous, nexts = _generate_stack_inputs(inst.heads, inst.type_ids, prior_order)
                if elmo:
...
def read_stacked_data_to_variable(source_path, word_alphabet, char_alphabet, pos_alphabet, type_alphabet, pos_embedding,
    ...
    for bucket_id in range(len(buckets)):
        ...
        inputidx = np.empty([bucket_size, bucket_length - 1], dtype=np.int64)
        # stack_reverse_inputidx = np.empty([bucket_size, bucket_length - 1], dtype=np.int64)
        answer_head = np.empty([bucket_size, bucket_length - 1], dtype=np.int64)
        answer_types = np.empty([bucket_size, bucket_length - 1], dtype=np.int64)
        ...
        for i, inst in enumerate(data[bucket_id]):    # data is a list
            ...
            if not is_test:
                ...
                tid_inputs[i, 0] = tids[0]
                tid_inputs[i, 1:inst_size] = stack_tids[::-1]
                tid_inputs[i, inst_size:] = PAD_ID_TAG
                # heads
                # hid_inputs[i, :inst_size] = hids
                hid_inputs[i, 0] = 0
                hid_inputs[i, 1:inst_size] = [inst_size - x if x != 0 else 0 for x in chids[::-1]]
            ...
            if not is_test:
                # inputidx
                inputidx[i, :inst_size_decoder] = stack_hids
                inputidx[i, inst_size_decoder:] = PAD_ID_TAG
                # reversed inputidx
                # stack_reverse_inputidx[i, :inst_size_decoder] = stack_hids[::-1]
                # stack_reverse_inputidx[i, inst_size_decoder:] = PAD_ID_TAG
                # answer heads
                answer_head[i, :inst_size_decoder] = [inst_size - x if x != 0 else 0 for x in chids[::-1]]
                # answer_head[i, :inst_size_decoder] = chids
                answer_head[i, inst_size_decoder:] = PAD_ID_TAG
                # answer types
                answer_types[i, :inst_size_decoder] = stack_tids[::-1]
                # answer_types[i, :inst_size_decoder] = stack_tids
                answer_types[i, inst_size_decoder:] = PAD_ID_TAG
                # previous
                # previous_inputs[i, :inst_size_decoder] = previous_ids[::-1]
                previous_inputs[i, :inst_size_decoder] = previous_ids
                previous_inputs[i, inst_size_decoder:] = PAD_ID_TAG
                # next
                # nexts_inputs[i, :inst_size_decoder] = nexts_ids[::-1]
        ...
        if volatile:
            with torch.no_grad():
                ...
                # decoder
                inputidx = Variable(torch.from_numpy(inputidx))
                # reverse_inputidx = Variable(torch.from_numpy(stack_reverse_inputidx))
                answer_head = Variable(torch.from_numpy(answer_head))
                answer_types = Variable(torch.from_numpy(answer_types))
        ...
        if use_gpu:
            ...
            # decoder
            inputidx = inputidx.cuda()
            # reverse_inputidx = reverse_inputidx.cuda()
            answer_head = answer_head.cuda()
            answer_types = answer_types.cuda()
        ...
        data_variable.append(((words, chars, pos, heads, types, masks_e, single, lengths_e),
                              (inputidx, answer_head, answer_types, previous, nexts, masks_d, lengths_d)))
...
def get_batch_stacked_variable(data, batch_size, pos_embedding, unk_replace=0., elmo=False, bert=False):
    ...
    # inputidx, reverse_inputidx, reverse_answer_head, reverse_answer_types, reverse_previous, reverse_nexts, masks_d, lengths_d = data_decoder
    inputidx, answer_head, answer_types, previous, nexts, masks_d, lengths_d = data_decoder
    bucket_size = bucket_sizes[bucket_id]
    ...
    return (words, chars[index], pos[index], heads[index], types[index], masks_e[index], lengths_e[index], word_bert_input), \
           (inputidx[index], answer_head[index], answer_types[index], previous[index], nexts[index], masks_d[index], lengths_d[index])


# dev, test load시 사용
def iterate_batch_stacked_variable(data, batch_size, pos_embedding, unk_replace=0., shuffle=False, type=None,
                                   elmo=False, bert=False):
    ...
    for bucket_id in bucket_indices:
        ...
        for start_idx in range(0, bucket_size, batch_size):
            ...
            yield (words[excerpt], chars[excerpt], pos[excerpt], heads[excerpt], types[excerpt], masks_e[excerpt], lengths_e[excerpt],
                input_word_bert[excerpt]), \
                (inputidx[excerpt], answer_head[excerpt], answer_types[excerpt], previous[excerpt],
                nexts[excerpt], masks_d[excerpt], lengths_d[excerpt])
