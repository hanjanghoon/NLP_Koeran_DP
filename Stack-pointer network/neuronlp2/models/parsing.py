...
class StackPtrNet(nn.Module):
    def __init__(self, word_dim, num_words, char_dim, num_chars, pos_dim, num_pos, num_filters, kernel_size,
        ...
        if self.bert:
            dim_enc = bert_feature_dim * 3 #+ 200
        if self.pos:
            dim_enc += 100
    ...
    def _get_encoder_output(self, input_word, input_char, input_pos, mask_e=None, length_e=None, hx=None, input_word_elmo=None, input_word_bert=None):
        ...
        if self.bert:
            ...
            _, output = resize_bert_output(bert_output, each_morp_lengths, each_eojeol_lengths,
                                           max_eojeol_length=max_eojeol_length, output_dim=self.hidden_size * 3,
                                           use_first_token=True, bert_word_feature_ids=bert_word_feature_ids,
        ...
        if self.elmo:
            ...
            sen_morphs = []
            mem_morphs = []
            for i, sentence in enumerate(input_word_elmo):  # 버켓네 전체 문장 *단어.
                sen_morph = []
                mem_morph = []
                k = 0
                for word in sentence:
                    mem_morph.append(k)  #### 이것은 단어의 첫번째 위치. 그래서 문장의 형태소 마다 단어의 시작 형태소 인덱스를 저장한다.
                    for morph in word:
                        sen_morph.append(morph)  ##sen_morph는 문장 ..
                        k += 1
                mem_morph.append(k)  # 마지막 인덱스.. 사실 형태소는 없지만. 끝부분을 의미한다.
                sen_morphs.append(sen_morph)  # 한문장이 끝이나면 문장들을 모아놓음.
                mem_morphs.append(mem_morph)  # 이것이 바로 문장에 대응되는 인덱스임..

            e = self.elmo_embedd.sents2elmo(sen_morphs)  # 이건 문장 * 형태소 * 1024짜리

            dim = list(output.size())
            elmo = torch.zeros([dim[0], dim[1], 2048],
                               dtype=torch.float32)  ####일반 쓰레기 값으로 초기화했다. dim[0] 는 아마 문장 dim1이 단어 ..
            elmo = elmo.cuda()
            for i in range(len(e)):
                mem_morph = mem_morphs[i]  # 봐봐 이게 i번째 문장이라는거야 .즉 memmorph는 한문장에서 시작 형태소 인덱스를 가진다는거임..
                for idx in range(len(mem_morph) - 1):  # 문장에서 단어의 시작 형태소를 적어놓은거야. 끝까지 안하는 이유는 한개 그냥 쓰레기값으로 초기해놨으니까...
                    elmo[i, idx, :1024] = e[i][mem_morph[idx]]  # 봐봐라 그러면, i번째 문장의 시작 형태소 idx값의 엘모값을..저장..
                    elmo[i, idx, 1024:] = e[i][mem_morph[idx + 1] - 1]  # 봐봐라 그러면, i번째 문장의 시작 형태소 idx값의 끝 엘모값을..저장..

            # elmo = self.elmo_linear(elmo) #kwon
            output = torch.cat([output, elmo], dim=2)
    ...
    def loss(self, input_word, input_char, input_pos, heads, stacked_heads, children, siblings, stacked_types, label_smooth,
        ...
        if input_word_elmo is not None:
            ...
            output_enc, hn, mask_e, _ = self._get_encoder_output(input_word, input_char, input_pos, mask_e=mask_e, length_e=length_e, hx=hx, input_word_bert=input_word_bert)
