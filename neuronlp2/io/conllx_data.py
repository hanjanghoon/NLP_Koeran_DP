...
# TODO: these...
NONE_POS =  b"_NONE_POS"
NONE_WORD = b"_NONE_WORD"
"""
...
# TODO: these...
NONE_POS = "_NONE_POS"
NONE_WORD = "_NONE_WORD"    # 2to3

_START_VOCAB = [PAD, ROOT, END]    # for word
...
NUM_SYMBOLIC_TAGS = 3    # FIXME: this is not important. right?

# QUESTION: what is buckets?
_buckets = [10, 15, 20, 25, 30, 35, 40, 50, 60, 70, 80, 90, 100, 140]
_no_buckets = [500] # big number!!

from .reader import CoNLLXReader


def create_alphabets(alphabet_directory, train_path, pos_embedding, data_paths=None, max_vocabulary_size=50000, embedd_dict=None,
                     min_occurence=0, normalize_digits=True):

    # NOTE: dev and test vocabs can be included
    def expand_vocab():
        vocab_set = set(vocab_list)
        for data_path in data_paths:
            # logger.info("Processing data: %s" % data_path)
            # with open(data_path, 'r') as file:
            with open(data_path, 'r', encoding="utf-8") as file:  # 2to3
                for line in file:
                    # line = line.decode('utf-8')    # line is unicode  # 2to3
                    line = line.strip()
                    if len(line) == 0:
                        continue
                    ...
                    tokens = line.split('\t')
                    #print(tokens[1])
                    token = utils.get_token(tokens[1])    # unicode
                    for char in token:
                        char_alphabet.add(char)

                        # word = utils.DIGIT_RE.sub(b"0", tokens[1]) if normalize_digits else tokens[1]
                    word = utils.DIGIT_RE.sub("0", tokens[1]) if normalize_digits else tokens[1]  # 2to3
                    word = word.split('|')    # list
                    ...
                    # NOTE: embedd_dict에 있는 단어만 등록!!
                    for w in word:
                        if w not in vocab_set and (w in embedd_dict or w.lower() in embedd_dict):
                            vocab_set.add(w)
                            vocab_list.append(w)
    ...
    pos_alphabet = Alphabet('pos', pos_embedding=pos_embedding)    # no unk
    type_alphabet = Alphabet('type')    # no unk
    if not os.path.isdir(alphabet_directory):
        logger.info("Creating Alphabets: %s" % alphabet_directory)

        char_alphabet.add(PAD_CHAR)    # 1
        pos_alphabet.add(PAD_POS)      # 0
        type_alphabet.add(PAD_TYPE)    # 0

        char_alphabet.add(ROOT_CHAR)    # 2
        pos_alphabet.add(ROOT_POS)      # 1
        type_alphabet.add(ROOT_TYPE)    # 1

        char_alphabet.add(END_CHAR)    # 3
        pos_alphabet.add(END_POS)      # 2
        type_alphabet.add(END_TYPE)    # 2

        pos_alphabet.add(NONE_POS)    # 3
        word_alphabet.add(NONE_WORD)  # 1

        vocab = dict()
        with open(train_path, 'r', encoding="utf-8") as file:  # 2to3
            for line in file:
                line = line.strip()
                if len(line) == 0:
                    continue
                ...
                tokens = line.split('\t')
                token = utils.get_token(tokens[1])    # token is unicode
                for char in token:
                    char_alphabet.add(char)
                ...
                for w in word:
                    if w in vocab:
                        ...
                        vocab[w] = 1
        # collect singletons
        singletons = set([word for word, count in list(vocab.items()) if count <= min_occurence])
        ...
        logger.info("Total Singleton Size:  %d" % len(singletons))    # 되게 안 나온 단어들!
        vocab_list = [word for word in vocab_list if word in _START_VOCAB or vocab[word] > min_occurence]    # 최소 빈도수 제한
        ...
        for word in vocab_list:
            word_alphabet.add(word)    # starting from 2
...
def read_data_to_variable(source_path, word_alphabet, char_alphabet, pos_alphabet, type_alphabet, pos_embedding, max_size=None,
                          normalize_digits=True, symbolic_root=False, symbolic_end=False,
...
def get_batch_variable(data, batch_size, pos_embedding, unk_replace=0.):
    ...
    if unk_replace:
        ...
        noise = Variable(masks.data.new(batch_size, bucket_length, pos_embedding).bernoulli_(unk_replace).long())
        words = words * (ones - single[index] * noise)

    return words, chars[index], pos[index], heads[index], types[index], masks[index], lengths[index]

def iterate_batch_variable(data, batch_size, pos_embedding, unk_replace=0., shuffle=False):
    data_variable, bucket_sizes = data
    ...
    for bucket_id in bucket_indices:
        ...
        if unk_replace:
            ...
            noise = Variable(masks.data.new(bucket_size, bucket_length, pos_embedding).bernoulli_(unk_replace).long())
